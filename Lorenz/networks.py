...
#         )
    ...
    def call(self, inputs, training=False):
        ...
        self.add_metric(self.latent_regularizer(embeddings), name="latent_regularizer", aggregation="mean")
...
class LSTMAutoencoder(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        embeddings =self.encoder(inputs) 
        outputs = self.decoder(embeddings)
        self.add_metric(self.latent_regularizer(embeddings),
         name="latent_regularizer", aggregation="mean")
        self.add_metric(self.latent_regularizer(embeddings)/self.latent_regularizer.strength,
         name="latent_regularizer_normalized", aggregation="mean")
        return outputs
    
    
class LSTMVAE(tf.keras.Model):
    """
    An LSTM VAE model for time series
    """
    def __init__(
        self,
        n_latent,
        ...
        **kwargs
        ...
        self.encoder.add(tfkl.Dense(tfpl.IndependentNormal.params_size(n_latent), 
                                 activation=None, name='z_params'))
        self.encoder.add(tfpl.IndependentNormal(n_latent, 
